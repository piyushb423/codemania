...
<h4 style="color:red"><marquee behavior=scroll scrollamount="8" direction="left">Questions you are tagged</marquee></h4>
<?php
  include("connection.php");
  $sql="select ID from tag where username='$usname'";
  $result1 = $conn->query($sql);
  $_SESSION['TOPIC']="notification.php";
  
  if ($result1->num_rows > 0 ) {
    while($row = $result1->fetch_assoc()) {
    $id=$row["ID"];
    $sql3="select question,username,qpoint from question where ID='$id' " ;
    $result = $conn->query($sql3);
    if ($result->num_rows > 0 ) {
  ...
  echo "<h3 align='center'>"."NO Question" . "</h3>"; 
}
?>
</div>
</div>
